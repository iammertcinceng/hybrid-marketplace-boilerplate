...
export default function ListingCard({
  ...
  return (
    <Card className="flex flex-col">
      <CardHeader>
        <a
          href={`/listing/${encodeURIComponent(listing.title.toLowerCase().replace(/[^a-z0-9]+/g, "-"))}-${listing.id}`}
      ...
      <CardContent className="flex-1">
        ...
        <TooltipProvider>
          <div className="flex justify-end space-x-2 mt-4">
            {!listing.active && onEdit && (
              // aktif olmayan ilanlar için düzenleme butonu
              <Tooltip>
                ...
                <TooltipContent
                  side="top"
                  className="bg-gray-900 text-white border-0 shadow-lg rounded px-2.5 py-1 text-xs"
                >
                  Düzenle
                </TooltipContent>
              </Tooltip>
