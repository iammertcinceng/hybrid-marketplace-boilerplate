    ...
    <Card className="p-0">
      <CardHeader className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div className="flex flex-col sm:flex-row sm:items-center gap-2">
          <CardTitle className="text-lg sm:text-xl">{viewTitles[viewMode]}</CardTitle>
          <span className={`text-xs font-medium px-2.5 py-0.5 rounded-full w-fit ${state[viewMode].color}`}>
            {state[viewMode].text} Modu
          </span>
        </div>
        {viewMode === 'list' ? (
          <Button onClick={handleAddNew} className="w-full sm:w-auto">Yeni Blog Ekle</Button>
        ) : (
          <Button variant="outline" onClick={handleBackToList} className="w-full sm:w-auto">Geri Dön</Button>
        )}
      </CardHeader>
      <CardContent>
