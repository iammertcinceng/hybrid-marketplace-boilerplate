...
export async function generateMetadata({
  ...
  const canonical = city
    ? `https://mertcin.com/category/${params.slug}/${city}`
    : `https://mertcin.com/category/${params.slug}`;
    ...
    "@graph": [
      // BreadcrumbList Schema
      {
        ...
        "itemListElement": breadcrumbs.map((item, index) => ({
          ...
          "position": index + 1,
          "name": item.name,
          "item": `https://mertcin.com${item.url}`
        }))
      },
      ...
      // ItemList Schema
      {
        ...
        "itemListElement": sortedListings.map((listing, index) => ({
          ...
          "item": {
            ...
            "url": `https://mertcin.com/listing/${createSeoUrl(listing.title)}-${listing.id}`
          }
        }))
      },
      ...
      {
        "@type": "WebSite",
        "url": "https://mertcin.com/",
        "potentialAction": {
          "@type": "SearchAction",
          "target": {
            "@type": "EntryPoint",
            "urlTemplate": "https://mertcin.com/category/${params.slug}?search={search_term_string}"
          },
        ...
        {!sortedListings?.length && listings?.length === 0 ? (
          ...
          sortedListings.map((listing) => (
            ...
            >
              ...
              <div className="flex items-center p-5 pb-4 border-b border-gray-100">
                ...
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-4 mt-1">
                    {listing.city && (
                      <Link
                        href={`/category/${category.parentId === null ? category.slug : params.slug
                          }/${listing.city
                            .toLowerCase()
                            .normalize("NFD")
                            .replace(/[\u0300-\u036f]/g, "")
              ...
              <div className="p-5">
                <Link
                  href={`/listing/${createSeoUrl(listing.title)}-${listing.id}`}
                ...
                <div className="flex items-center justify-between pt-4 border-t border-gray-100">
                  <div className="flex items-center gap-3">
                    {category.parentId === null && (
                      <Link
                        href={`/category/${categories?.find((c) => c.id === listing.categoryId)?.slug || ""
                          }`}
      ...
      {sortedListings.length > 0 && (
        <div className="mt-8">
          <div className="flex items-center justify-center gap-2">
            {page > 1 && (
              <Link
                href={`/category/${params.slug}${city ? `/${city}` : ""}${page === 2 ? "" : `?page=${page - 1}`
            ...
            {Array.from(
              ...
              (_, i) => {
                ...
                return (
                  <Link
                    key={pageNum}
                    href={`/category/${params.slug}${cityPath}${pagePath}${searchPath}`}
                    className={cn(
            ...
            {page < Math.ceil(total / 10) && (
              <Link
                href={`/category/${params.slug}${city ? `/${city}` : ""}?page=${page + 1}${search ? `&search=${search}` : ""
                  }`}
                className="px-4 py-2 rounded-md bg-white hover:bg-gray-100"
              >
                →
