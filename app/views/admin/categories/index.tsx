...
export default function KategorilerPage() {
  ...
  const reorderMutation = useMutation({
    ...
    onMutate: async (updates) => {
      ...
      // Yeni sıralamayı UI'da hemen göster
      queryClient.setQueryData(["categories"], (old: CategoryWithChildren[] | undefined) => {
        if (!old) return old;
        
        // Güncellenecek kategorilerin parent'ını bul
        const parentId = updates[0]?.parentId;
        
        if (parentId === null) {
          // Ana kategoriler güncelleniyor
          const updatedCategories = [...old];
          updates.forEach(update => {
            const categoryIndex = updatedCategories.findIndex(cat => cat.id === update.id);
            if (categoryIndex !== -1) {
              updatedCategories[categoryIndex] = { ...updatedCategories[categoryIndex], order: update.order };
            }
          });
          return updatedCategories.sort((a, b) => (a.order ?? 0) - (b.order ?? 0));
        } else {
          // Alt kategoriler güncelleniyor
          return old.map(mainCat => {
            if (mainCat.id === parentId) {
              const updatedChildren = [...mainCat.children];
              updates.forEach(update => {
                const childIndex = updatedChildren.findIndex(child => child.id === update.id);
                if (childIndex !== -1) {
                  updatedChildren[childIndex] = { ...updatedChildren[childIndex], order: update.order };
                }
              });
              return { ...mainCat, children: updatedChildren.sort((a, b) => (a.order ?? 0) - (b.order ?? 0)) };
            }
            return mainCat;
          });
        }
    ...
    onSuccess: () => {
      // Başarılı olduğunda sadece cache'i yenile (optimistic update zaten yapıldı)
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (updates.length > 0) {
        // Optimistic update ile hemen UI'ı güncelle
  ...
  return (
    <div>
      <h1 className="text-xl sm:text-2xl font-bold mb-6 sm:mb-8">Kategoriler</h1>

      <div className="mb-6">
        <h2 className="text-base sm:text-lg font-semibold mb-2">Yeni Kategori Ekle</h2>
        <div className="flex flex-col sm:flex-row gap-3">
          <Input
            ...
            onChange={(e) => {
              ...
              setNewCategory({ 
                ...newCategory, 
                name,
                slug: slugify(name) // Otomatik slug oluştur
              });
            }}
          ...
          <Button onClick={() => addMutation.mutate(newCategory)} className="w-full sm:w-[120px] sm:px-6">Ekle</Button>
        </div>
      </div>
