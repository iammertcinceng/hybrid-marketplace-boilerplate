...
export default function MessagesView({
  ...
  const otherUserId = React.useMemo(() => {
    // Prefer explicit participant from parent conversation prop
    if (conversation) {
      if (type === "sent") {
        return (
          (conversation as any).receiverId ||
          (conversation as any).receiver?.id ||
          null
        );
      } else {
        return (
          (conversation as any).senderId ||
          (conversation as any).sender?.id ||
          null
        );
      }
    }

    // Fallback: infer from first message and current user
    const firstPage = data?.pages?.[0];
    const firstMessage = firstPage?.messages?.[0];
    if (firstMessage && currentUserId) {
      return firstMessage.senderId === currentUserId
    ...
    return conversationStarterId ?? null;
  }, [conversation, type, data, currentUserId]);
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
              ...
              {!isLoading ? (
                <>
                  {isAdminConversation && conversation?.sender?.isAdmin
                    ...
                    : type === "sent"
                    ? (
                        (conversation?.contactPerson && conversation.contactPerson.trim().length > 0)
                          ? conversation.contactPerson
                          : (listing?.contactPerson && listing.contactPerson.trim().length > 0)
            ...
            {data?.pages?.[0]?.listingId && (
              <Link
                href={`/listing/${data?.pages?.[0]?.listingId}`}
                target="_blank"
                rel="noopener noreferrer"
              ...
              >
                ...
                {conversation?.listingTitle ?? listing?.title ?? "İlana Git"}
              </Link>
            )}
          </div>
        </div>
              ...
              >
                ...
                <div className="text-xs font-semibold mb-1">
                  {message.senderId === currentUserId
                    ? "Siz" + (conversation?.contactPerson && conversation.contactPerson.trim().length > 0 ? " (" + conversation.contactPerson + ")" : "")
                    : isAdminConversation && conversation?.sender?.isAdmin
                    ? "YÖNETİM"
                    : type === "sent"
                    ? (
