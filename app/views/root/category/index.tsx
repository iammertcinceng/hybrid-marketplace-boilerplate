    ...
    <div className="mx-auto bg-white p-6 rounded-lg shadow-lg mb-8">
      <form
        onSubmit={(e) => {
          ...
          const url = `/category/${category}${normalizedCity}`;
              ...
              {categories
                ...
                .map((mainCategory) => (
                  <SelectGroup key={mainCategory.id}>
                    {/* <SelectLabel className="font-semibold text-sm text-red-700">
                      {mainCategory.name}
                    </SelectLabel> */}
                    {/* Farketmez/Hepsi seçeneği - sadece ana kategoriler için */}
                    <SelectItem className="font-semibold" value={mainCategory.slug}>
        ...
        <input
          ...
          onKeyDown={(e) => {
            if (e.key === "Enter") {
              ...
              const baseUrl = `/category/${params.slug}${cityParam}`;
