...
// FACEBOOK LOGİN DEVREDIŞI: import FacebookProvider from "next-auth/providers/facebook";
...
export const authOptions: NextAuthOptions = {
  providers: [
    CredentialsProvider({
      ...
      async authorize(credentials) {
        ...
        try {
          ...
          // Environment variable'dan secret key'i al
          const secretKey = process.env.RECAPTCHA_SECRET_KEY;
          if (!secretKey) {
            throw new Error("reCAPTCHA secret key tanımlanmamış");
          }
          console.log('Kullanılan secret key (env):', secretKey.substring(0, 5) + '...');
    ...
    // FACEBOOK LOGİN DEVREDIŞI: Aşağıdaki Facebook provider devre dışı bırakıldı
    // FacebookProvider({
  ...
  callbacks: {
    async signIn({ user, account, profile }) {
      ...
      if (account?.provider === 'google' /* FACEBOOK LOGİN DEVREDIŞI: || account?.provider === 'facebook' */) {
        ...
        if (existingUser) {
          ...
          // FACEBOOK LOGİN DEVREDIŞI: Facebook id güncellemesi devre dışı
          ...
          // FACEBOOK LOGİN DEVREDIŞI: Facebook id ataması devre dışı
