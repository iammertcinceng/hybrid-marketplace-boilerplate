...
import { generateNewMessageEmail } from './shared/services/email-templates';
      ...
      if (conversation) {
        try {
          ...
          if (receiverSocket) {
            ...
            // Kullanıcı çevrimdışı ise e-posta gönder
            try {
              ...
              if (receiver?.email && receiver.username && sender?.username) {
                const inboxUrl = `${config.appUrl}/inbox`;
                const tpl = generateNewMessageEmail(
                  receiver.username,
                  sender.username,
                  content || (files && files.length ? 'Yeni bir dosya aldınız' : ''),
                  inboxUrl,
                );
                await sendEmail({
                  to: receiver.email,
              ...
              console.error('Yeni mesaj e-posta bildirimi gönderilemedi:', err);
            }
          }
        ...
        try {
          ...
          // Alıcıya özel bildirim gönder (yeni konuşma senaryosu)
          const receiverSocket = Array.from(io.sockets.sockets.values()).find(
            (s: any) => s.userId === receiverId.toString()
          );
          ...
          if (receiverSocket) {
            ...
            try {
              ...
              console.error('Yeni mesaj e-posta bildirimi (yeni konuşma) gönderilemedi:', err);
            }
          }
